 ...
 * receive ByteBuffer based access via org.rocksdb.DirectSlice
 * in their merge method implementation.
 *
 * ByteBuffer based slices perform better when large keys
 * are involved. When using smaller keys consider
 * using @see org.rocksdb.AssociativeMergeOpr
 */
public abstract class DirectAssociativeMergeOpr extends DirectMergeOpr {
   ...
   */
  public final boolean fullMerge(DirectSlice key,
		  DirectSlice existing_value,
          Deque<DirectSlice> operand_list,
          DirectSlice new_value) {
	  return true;
  }
  ...
  private native void createNewDirectAssociativeMergeOpr0(final long mergeOprOptionsHandle);
}
